...
Morphling.optionIcon = Menu.AddOptionIcon({ Hero Specific, Morphling }, panoramaimagesheroesiconsnpc_dota_hero_morphling_png.vtex_c)
...
function Morphling.init_heroes_dmg(myHero, eblade, strike)
	...
	local ebladeOnly = ebladeDmgintMultiplierebladeMultiplier
...
function Morphling.getStrikeDmg(agility, strenght, strike)
	...
	local maxMultiplier = 0.5 + (strikeLevel0.5)
	local totalAttributies = agility + strenght
	if (totalAttributies10075  agility) then 
		damageMultiplier = maxMultiplier
	else
		damageMultiplier = minMultiplier
	end
	strikeDmg = 100 + (agilitydamageMultiplier)
	return strikeDmg
end
...
function Morphling.IsHasGuard(npc) 
	...
	if abaddonUlt and Ability.GetLevel(abaddonUlt)0 then
			...
			elseif Ability.SecondsSinceLastUse(abaddonUlt)=1 then --??? ???? ????? ??????, ?? ???? ???????, ????? ?????? ??? ? ??, ? ???????????? ??? ???.
				guarditis = Immune
			end
	end
	...
	if aeon_disk and (Ability.IsReady(aeon_disk) or Ability.SecondsSinceLastUse(aeon_disk)=1) then guarditis = Immune end --???? ??????, ??? ? ? ?????????
	return guarditis
end
...
function Morphling.GetTotalDmg(target,dmg, myHero)
	...
	if visageCloak and visageCloak ~= 0 then
		...
		totalDmg = totalDmg  (1 - (0.2visageStack))
	end
...
function Morphling.MorphDrawBalanceBoard(myHero)
	...
	-- colored rect
	for i = 1, 20 do
		Renderer.SetDrawColor(25 + i10, 230 - i10, 0, 150)
	...
	if Input.IsKeyDownOnce(Enum.ButtonCode.MOUSE_LEFT) then
		for i, v in ipairs(hoveringTable) do
			...
			elseif hoveringTable[20] == true then
				...
				if v == true then
					Morphling.MorphBalanceSelectedHP = minHP + HPstepsi
